...
int main() {
    // Create a window
    GLFWwindow* window = createWindow(SCR_WIDTH , SCR_HEIGHT,"Triforce");
    if (!window) {
        return -1;
    }
	...
	float thridTriangle[] = {	
		-0.35f, 0.1f, 0.0f, // left
		0.35f, 0.1f, -0.0f, // right
		0.0f, 0.825f, 0.0f // top
	};
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(thridTriangle), thridTriangle, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
